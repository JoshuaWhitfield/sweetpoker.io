...
const haskey = function(item=false, obj) { //Object.prototype.
    if (!obj) {console.log('not an obj');return false}
    if (item == false) {console.log('invalid item');return false}
    console.log('checkpoint 2: '+Object.keys(obj))
    if ( arr_has(item, Object.keys(obj)) ) return true
    return false
};
...
deck.generate = function() {
    ...
    for (let suit_name of suits) {
        for (var i=1;i<=13;i++) {
        let name="";
        console.log('checkpoint: '+haskey(i.toString() ,fctv))
        if (haskey(i.toString() ,fctv)) {name = fctv[i]+"_"+suit_name; continue}
...
export const determine_hand = function (river, hand=false) {
    ...
    var get_instances = function (vl) { //this function records every instance of a card via value and returns a map: {"card_value": instance_int, ...}
        var instances = {};
        for (let item of vl) {
            if (!haskey(item, instances)) {instances[item]=1;continue}
            instances[item]+=1
        }
        return instances
    }
