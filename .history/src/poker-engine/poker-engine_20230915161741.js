const str_has = function (comparison) { //String.prototype.
    if (this.indexOf(comparison) < 0) {return false}
    return true
};
const haskey = function(item=false, obj) { //Object.prototype.
    if (!obj) return false
    if (item == false) {return false}
    if (item in this) {return true}
    return false
};
const shuffleArr = function () { //Array.prototype.
    let new_arr=[]
    for (var i=1; i <=5 ;i++) {
...
const arr_has = function (comparison) { //Array.prototype.
    if (this.indexOf(comparison) < 0) {return false}
    return true
};
...
deck.generate = function() {
    ...
    for (let suit_name of suits) {
        for (var i=1;i<=13;i++) {
        let name="";
        if (haskey(fctv ,i)) name = fctv[i]+"_"+suit_name
...
deck.river.deal = function () {
    ...
    for (var i=1; i <=5 ;i++) {
        let card = deck.cards.shuffle()[rnd()]  //shuffle(): shuffles the deck five times per call
...
deck.generate()
//export let officialDeck = 
export let river = deck.river.deal
export let pocket = deck.hand.deal
